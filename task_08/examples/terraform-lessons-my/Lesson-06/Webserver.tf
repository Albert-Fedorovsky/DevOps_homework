...
resource "aws_instance" "my_webserver" {
  ...
  user_data = templatefile("user-data.sh.tpl", {
    f_name = "Albert",
    l_name = "Fedorovsky",
    names  = ["Vasya", "Kolya", "Petya", "Jonh", "Donald", "Masha", "Katya"]
  })
  ...
  lifecycle {
    # prevent_destroy = true # terraform запрещает любые пересоздание этого инстанса при очередном apply
    # ignore_changes = [ami, user_data] # terraform игнорирует изменения указанных полей при очередном apply
    create_before_destroy = true # terraform сначала создаёт новый инстанс, и только затем унечтожает старый при очередном apply
  }
